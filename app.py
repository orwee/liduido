...
# ==============================================================================
# Contenido de la Pestaña 1: ANÁLISIS HISTÓRICO APY
# ==============================================================================
with tab1:
...
# ==============================================================================
# Contenido de la Pestaña 2: ANÁLISIS DE PRICE IMPACT
# ==============================================================================
with tab2:
    ...
    # --- Funciones de API (no necesitan cambios) ---
    def fetch_pools(tokenA, tokenB, log_area):
        ...
        r.raise_for_status()
        j = r.json()
        if isinstance(j, dict) and j.get("success") is False: raise RuntimeError(f"Error API /pools: {j}")
        return (j.get("data", []) if isinstance(j, dict) else j)

    def call_route(tokenIn, tokenOut, amountIn, log_area, excludeDexes=None):
    ...
    # --- Función principal de análisis (MODIFICADA para aceptar amounts) ---
    def analyze_pair(tokenA, tokenB, pair_name, inverse_flag, log_area, amounts_to_analyze):
        rows = []
        try:
        ...
        for p in pools:
            ...
            base_row = {"pair": pair_name, "pair_tokenA": tokenA, "pair_tokenB": tokenB, "inverse": inverse_flag, "poolAddress": poolAddress, "protocol": protocol, "routerIndex": routerIndex}
    ...
    uploaded_file = st.file_uploader("Sube tu archivo CSV con los pares de Gliquid.", type="csv")
        ...
        try:
            ...
            if not all(c in df_tokens.columns for c in needed_cols):
                ...
                if df_gliquid.empty:
                    ...
                    selected_pair = st.selectbox("Selecciona el Par a analizar:", pair_options)

                    amounts_input = st.text_input("Define los montos (amounts) a simular (separados por coma):", "0.1, 1, 10, 100, 1000, 10000, 100000")
                    
                    if st.button(f"🚀 Iniciar Análisis para {selected_pair}"):
                        # --- LÓGICA DE ANÁLISIS PARA UN SOLO PAR ---
                        
                        # 1. Validar y procesar los amounts
                        try:
                            amounts_to_analyze = [float(x.strip()) for x in amounts_input.split(',')]
                            if not amounts_to_analyze:
                                raise ValueError("La lista de montos no puede estar vacía.")
                        except ValueError as e:
                            st.error(f"Error en el formato de los montos. Asegúrate de que sean números separados por comas. Detalles: {e}")
                            st.stop()
                        
                        # 2. Obtener datos del par seleccionado
                        ...
                        with st.spinner(f"Analizando '{pair_name}'... Esto puede tardar unos segundos."):
                            # Normal
                            log_area.info(f"Ejecutando {pair_name}: {tokenA} / {tokenB} (inverse=NO)")
                            rows_normal = analyze_pair(tokenA, tokenB, pair_name, "NO", log_area, amounts_to_analyze)
                            all_rows.extend(rows_normal)
                            
                            # Inverso
                            log_area.info(f"Ejecutando {pair_name}: {tokenB} / {tokenA} (inverse=YES)")
                            rows_inverse = analyze_pair(tokenB, tokenA, pair_name, "YES", log_area, amounts_to_analyze)
                        ...
                        if not all_rows:
                            ...
                            amount_cols = [f"amount_{amt}_avgPriceImpact" for amt in amounts_to_analyze]
                            meta_cols = ["pair", "pair_tokenA", "pair_tokenB", "inverse", "poolAddress", "protocol", "routerIndex"]
                            ...
                            st.download_button(
                                ...
                                file_name=f"PriceImpact_{pair_name.replace('/', '-')}_{datetime.now().strftime('%Y%m%d')}.csv",
                                mime="text/csv"
                            )
        except Exception as e:
            st.error(f"No se pudo procesar el archivo CSV. Asegúrate de que sea un archivo válido. Error: {e}")
